...
div[data-testid="metric-container"] label { width: 100%; text-align: center; display: block; }
div[data-testid="metric-container"] div[data-testid="stMetricValue"] { justify-content: center; }
...
@st.cache_data
def load_data():
    file_path = "data/github_language_stats.parquet"
    ...
    url = "https://github.com/yourusername/yourrepo/raw/main/data/github_language_stats.parquet"  # <-- replace with your raw GitHub URL
...
st.download_button("Download Dataset", csv, "github_language_stats.csv", "text/csv")
...
# GitHub Language Popularity Chart
...
for bar in bars:
    value = bar.get_width()
    ax.text(value * 0.98, bar.get_y() + bar.get_height()/2, star_formatter(value, None),
...
for bar in bars:
    value = bar.get_width()
    ax2.text(value * 0.98, bar.get_y() + bar.get_height()/2, f"{value:.1f}%",
            va="center", ha="right", color="white")
st.pyplot(fig2)
...
for count, patch in zip(counts, patches):
    if count > 0:
        ax3.text(patch.get_x() + patch.get_width()/2, count * 0.8, int(count),
                 ha="center", va="center", color="white")
st.pyplot(fig3)

# Technology Domains Chart
...
for bar in bars:
    value = bar.get_width()
    ax4.text(value * 0.98, bar.get_y() + bar.get_height()/2, f"{value:.1f}%",
            va="center", ha="right", color="white")
st.pyplot(fig4)
