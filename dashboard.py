...
@st.cache_data
def load_data():
    return pd.read_parquet("data_lake/gold/github_language_stats.parquet")
...
table_html = table_df[
    ["Rank","Language","Repositories","Total Stars"]
].to_html(index=False)
...
usage_summary = (
    usage_df.groupby("tech_usage")["total_stars"]
    .sum()
